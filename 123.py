...
class Runthread(QThread):
    ...
    def run(self):
        url=self.data
        page_now=1
        self.test(url,page_now)
        print(1+2)

class qt():
    def __init__(self):
        super(qt, self).__init__()
        self.timeThread = Runthread()
    ...
    def start(self):        #定义初始函数
        ...
        if requests.get(url+str(page_now)+'.png').status_code!=200:
            QMessageBox.information(self.win, "提示", "爬虫状态码为{}".format(requests.get(url + str(page_now) + '.png').status_code))
            return 0
        self.timeThread.data=url
        self.timeThread.pathSignal.emit(path)
